    ...
    getGog: (req,res)=>{
        (async () => {
            try {
                ...
                let imgUrl = document.querySelector('.product-tile.product-tile--grid source').srcset.split(',')[0]
                let price =  document.querySelector('price-value').textContent.trim().split('$').splice(1)
                let discount = document.querySelector('price-value').textContent.trim().split('$').splice(1).length === 1 ? '' : document.querySelector('price-discount').textContent.trim()
                let url = document.querySelector('.product-tile.product-tile--grid').href
                const data = {
                    name,
                    imgUrl,
                    price,
    ...
    getInstantGaming: (req,res)=>{
        (async () => {
            try {
                ...
                let name = document.querySelector('.search .item .title').textContent
                let imgUrl = document.querySelector('.search .item .picture').attributes[1].textContent
                let price =  document.querySelector('.search .item .price').textContent.split('€')[0]
                let discount = document.querySelector('.search .item a .discount').textContent
